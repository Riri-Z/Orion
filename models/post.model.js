...
module.exports = class Post {
    ...
    static create(newPost) {
        return db.query('INSERT INTO post_pos(pos_content, pos_createdAt, pos_updatedAt, pos_deletedAt, id_exa, id_usr, id_parent) VALUES (?, ?, ?, ?, ?, ?, ?)' , [
            newPost.pos_content,
    ...
    static update(id, post) {
        post.id_pos = id;
        return db.query('UPDATE post_pos SET pos_content = ?, pos_createdAt = ?, pos_updatedAt = ?, pos_deletedAt = ?, id_exa = ?, id_usr = ?, id_parent = ? WHERE id_pos = ?', [
            post.pos_content,
            post.pos_createdAt,
            post.pos_updatedAt,
            post.pos_deletedAt,
